...
(function () {
  ...
  /* --------------------------------------------------------------------
   * PHASE 30-HOTFIX-2 — RELIABLE UPDATE DETECTION (carried over from
   * Phase 29-Hotfix-2; see that phase's report for the full root-cause
   * evidence trail). Re-checks for a new Service Worker periodically and
   * on tab-focus/online, instead of only once per fresh page load, so a
   * long-lived background tab still reliably sees the update banner —
   * without ever auto-applying the update (still requires the person's
   * own click on "تحديث الآن", per this file's original design above). */
  var UPDATE_CHECK_INTERVAL_MS = 20 * 60 * 1000; // 20 minutes
